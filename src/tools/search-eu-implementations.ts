...
export interface SearchEUImplementationsInput {
  ...
  has_finnish_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  // Filter by implementation existence (Finnish alias preferred, Swedish kept for compatibility)
  const hasImplementation = input.has_finnish_implementation ?? input.has_swedish_implementation;
