...
import {
  normalizeChapterToken,
  normalizeProvisionReference,
...
export async function getProvision(
  ...
  let provisionRef = input.provision_ref
    ? normalizeProvisionReference(input.provision_ref)
    : undefined;

  if (!provisionRef) {
    if (input.chapter && input.section) {
      const chapter = normalizeChapterToken(input.chapter);
      const section = normalizeSectionToken(input.section);
      provisionRef = chapter && section ? `${chapter}:${section}` : undefined;
    } else if (input.section) {
      provisionRef = normalizeProvisionReference(input.section);
    }
  }
  ...
  if (!provisionRef) {
    return {
      results: getAllProvisions(db, resolvedDocumentId, asOfDate),
  ...
  if (asOfDate) {
    const sql = `
      ...
      LIMIT 1
    `;
    row = db.prepare(sql).get(resolvedDocumentId, provisionRef, asOfDate, asOfDate) as ProvisionRow | undefined;
  } else {
    const sql = `
      SELECT
        lp.document_id,
    ...
    `;
    row = db.prepare(sql).get(resolvedDocumentId, provisionRef) as ProvisionRow | undefined;
  }
  ...
  `).all(resolvedDocumentId, provisionRef) as CrossRefResult[];
