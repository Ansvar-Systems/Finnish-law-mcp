...
const STATUTE_ID_PATTERN = '(?:\\d{4}:\\d+|\\d+/\\d{4})';
const CHAPTER_WORD_PATTERN = '(?:kap\\.?|kapitel|luku)';
const SECTION_WORD_PATTERN = '(?:§|pyk[aä]l[aä]|paragraf(?:en)?)';

/** Statute pattern: SFS 2018:218 / Laki 1050/2018 [3 kap.|3 luku] [5 [a] §] */
const SFS_PATTERN = new RegExp(
  `^(?:SFS|Laki|Lag)?\\s*(${STATUTE_ID_PATTERN})\\s*(?:(\\d+)\\s*${CHAPTER_WORD_PATTERN}\\s*)?(?:(\\d+\\s*[a-z]?)\\s*${SECTION_WORD_PATTERN})?`,
  'iu'
);

/** Short form statute pattern: 2018:218 3:5 or 1050/2018 3:5 */
const SFS_SHORT_PATTERN = new RegExp(
  `^(?:SFS|Laki|Lag)?\\s*(${STATUTE_ID_PATTERN})\\s+(\\d+):(\\d+\\s*[a-z]?)\\s*$`,
  'iu'
);
...
export function detectDocumentType(citation: string): DocumentType | null {
  ...
  if (/^(?:(?:sfs|laki|lag)\s+)?(?:\d{4}:\d+|\d+\/\d{4})/u.test(trimmed)) return 'statute';
  return null;
}
