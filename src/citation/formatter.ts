/**
 * Format Finnish legal citations per standard conventions.
 */

import type { ParsedCitation, CitationFormat } from '../types/index.js';

/**
 * Format a parsed citation into a standard Finnish citation string.
 *
 * Formats:
...
function formatStatute(citation: ParsedCitation, format: CitationFormat): string {
  ...
  const isFinnishStyle = /^\d+\/\d{4}$/u.test(document_id);
  const chapterLabel = isFinnishStyle ? 'luku' : 'kap.';

  if (format === 'pinpoint') {
    if (chapter && section) return `${chapter} ${chapterLabel} ${section} §`;
  ...
  // full format
  let result = `${isFinnishStyle ? 'Laki' : 'SFS'} ${document_id}`;
  if (chapter) result += ` ${chapter} ${chapterLabel}`;
