#!/usr/bin/env node
/**
 * Finnish Legal Citation MCP Server
 *
 * Provides 8 tools for querying Finnish statutes, case law,
...
import { getFinnishImplementations, GetFinnishImplementationsInput } from './tools/get-finnish-implementations.js';
...
const DB_ENV_VAR = 'FINNISH_LAW_DB_PATH';
...
Returns matched provisions with snippets, relevance scores, and document metadata.`,
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Search query in Finnish or English. Supports FTS5 syntax.',
        },
        document_id: {
          type: 'string',
          description: 'Filter to a specific statute by statute number (e.g., "1050/2018" or "2018:218")',
        },
        status: {
          type: 'string',
...
Omit chapter/section/provision_ref to get all provisions in the statute.`,
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description: 'statute number (e.g., "1050/2018" or "2018:218")',
        },
        chapter: {
          type: 'string',
...
Essential for understanding legislative intent behind statutory provisions.`,
  ...
  },
  {
    ...
    description: `Validate a Finnish legal citation against the database.
...
Searches across statutes, case law, and preparatory works simultaneously to aggregate relevant citations. Use this for broad legal research questions.`,
  ...
  },
  {
    ...
    description: `Format a Finnish legal citation per standard conventions.
...
Use this for exploratory searches like "data protection" or "privacy" to find relevant EU law.`,
    inputSchema: {
      ...
      properties: {
        ...
        has_finnish_implementation: {
          type: 'boolean',
          description: 'Filter by Finnish implementation existence',
        },
        limit: {
          type: 'number',
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  ...
  return {
    resources: [
      {
        uri: 'case-law-stats://finnish-law-mcp/metadata',
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (uri === 'case-law-stats://finnish-law-mcp/metadata') {
    try {
      const db = getDb();
          ...
          contents: [
            {
              ...
              text: JSON.stringify(
                {
                  status: 'no_data',
                  message: 'No case law data has been synced yet. Ingest case law separately and rebuild the database.',
                  last_sync_date: null,
                  ...
                  source: {
                    ...
                    url: 'https://finlex.fi',
                    license: 'Open data',
                    attribution: 'Case law should be verified against official Finlex publications.',
                  },
  ...
  try {
    ...
    switch (name) {
      ...
      case 'get_finnish_implementations':
        result = await getFinnishImplementations(getDb(), args as unknown as GetFinnishImplementationsInput);
        break;
